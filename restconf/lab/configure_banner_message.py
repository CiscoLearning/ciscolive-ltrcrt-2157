...
with open(BANNER_TEXT_FILE, "r", encoding="utf-8") as file:
    banner = file.# <TODO_1> add a method that reads the content of the file
...
for device_name, device in testbed.devices.items():
    ...
    device.connect(via="<TODO_2>") # add the rest connection method
    ...
    rest_payload = device.api.load_jinja_template(
            path=TEMPLATE_PATH,
            file="<TODO_3>", # add the name of the jinja2 file
            <TODO_4>=banner, # add the variable you used in the template
            )
    try:
